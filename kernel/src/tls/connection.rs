...
use super::constants::{HEADER_LEN, IN_BUF_SIZE, MAX_TLS_RECORD_LEN, OUT_BUF_SIZE};
...
impl TlsClient {
    ...
    pub fn connect(
        ...
        Ok(TlsConnection::new(
            sock,
            Box::new(
                UnbufferedClientConnection::new(
                    self.config.clone(),
    ...
    fn create_client_config(debug_info: bool) -> Result<Arc<ClientConfig>, TlsError> {
        let crypto_provider = Arc::new(provider());
        let time_provider = Arc::new(FixedTimeProvider::december_2025());
        ...
        if debug_info {
            Arc::get_mut(&mut config)
                .expect("Should have exclusive access to config")
                .key_log = Arc::new(super::key_logger::MyKeyLogger);
        }
...
pub struct TlsConnection {
    conn: Box<UnbufferedClientConnection>,
    sock: VsockStream,
    in_buffer: TlsBuffer,
...
impl TlsConnection {
    ...
    pub fn new(sock: VsockStream, conn: Box<UnbufferedClientConnection>) -> Self {
    ...
    pub fn complete_handshake(&mut self) -> Result<(), SvsmError> {
        ...
        if self.status.handshake_complete {
            return Err(TlsError::HandshakeAlreadyComplete.into());
        }
    ...
    pub fn read_tls(&mut self, buf: &mut [u8]) -> Result<usize, SvsmError> {
        ...
        if !self.status.handshake_complete {
            return Err(TlsError::HandshakeNotComplete.into());
        }
    ...
    pub fn write_tls(&mut self, data: &[u8]) -> Result<(), SvsmError> {
        ...
        if data_len == 0 || data_len > MAX_TLS_RECORD_LEN {
            return Err(TlsError::InvalidRecordLength.into());
        }
    ...
    fn tls_state_machine_advance(&mut self, action: TlsAction<'_>) -> Result<(), SvsmError> {
        ...
        log::info!("TLS State Machine State: {:?}", state);
