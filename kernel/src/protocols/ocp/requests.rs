...
use super::source::{OCP_SOURCE_DETAILS_SIZE, OCP_SOURCE_ENTRY_SIZE, OcpObjectDetails, OcpSource};
...
fn ocp_list_object_sources_request(params: &mut RequestParams) -> Result<(), SvsmReqError> {
    ...
    // compute it based on the number of entries and
    // the size of each entry
    let buffer_size = num_entries * OCP_SOURCE_ENTRY_SIZE;
    ...
    for (i, entry) in object.get_object_sources().iter().skip(first).enumerate() {
        if i >= num_entries {
            break;
        }
        // SAFETY: guest_entry is obtained from an untrusted GPA.
