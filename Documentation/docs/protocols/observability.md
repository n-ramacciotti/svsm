...
* primary index: Represents the object in which the source is
  located
* secundary index: Represents the index at which the source is
  available through the object
* type: kind of data returned from this source. Such as string
  or integer values.
* flags: contains sources information.  For now, it only
  indicates whether or not a source is writable.
